...
if(array_key_exists('GetInfo', $_POST)){
	...
	if(is_int($number)){
		...
		$sql = "select Mailed.issuenumber, Mailed.deliverystatus, Mailed.receiverstatus, HasShipmentMethod.duration
		from Mailed inner join HasShipmentMethod on Mailed.issuenumber = HasShipmentMethod.issuenumber
		...
		if(is_object($result)){
			...
			echo "Please enter a valid issue number. No such entry exists. ";
		}
	}
